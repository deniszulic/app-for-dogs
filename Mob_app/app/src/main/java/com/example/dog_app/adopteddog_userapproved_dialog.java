...
public class adopteddog_userapproved_dialog extends BottomSheetDialogFragment {
...
//        napomena_nestanak_myreports_missingdogs=(TextInputLayout) view.findViewById(R.id.azilnapomena_userreq_adopteddog);
        ...
        if(user_req_adoptdog_approved_adapter.getPrihvaceno().equals("true")){
            radio_button_1_userreq_missingdog.setChecked(true);
        }
        if(user_req_adoptdog_approved_adapter.getPrihvaceno().equals("false")){
...
//        }
        ...
        ime_nestanak_myreports_missingdogs.getEditText().setText(user_req_adoptdog_approved_adapter.getIme());
        prezime_nestanak_myreports_missingdogs.getEditText().setText(user_req_adoptdog_approved_adapter.getPrezime());
        telefonskibr_myreports_missingdogs.getEditText().setText(user_req_adoptdog_approved_adapter.getTelefonskibr());
        grad_azila_myreports_missingdogs.getEditText().setText(user_req_adoptdog_approved_adapter.getGrad());
        postanski_broj_myreports_missingdogs.getEditText().setText(user_req_adoptdog_approved_adapter.getPostanski_broj());
        azilnapomena_userreq_adopteddog.getEditText().setText(user_req_adoptdog_approved_adapter.getAzil_udomljavanje_napomena());
        dlaka_myreports_missingdogs.getEditText().setText(user_req_adoptdog_approved_adapter.getDlaka());
        spol_myreports_missingdogs.getEditText().setText(user_req_adoptdog_approved_adapter.getSpol());


        adresa_myreports_missingdogs.getEditText().setText(user_req_adoptdog_approved_adapter.getAdresa());
        imepsa_myreports_missingdogs.getEditText().setText(user_req_adoptdog_approved_adapter.getIme_psa());
        pasmina_myreports_missingdogs.getEditText().setText(user_req_adoptdog_approved_adapter.getPasmina());
        starost_myreports_missingdogs.getEditText().setText(user_req_adoptdog_approved_adapter.getStarost());
        vetlokacija_myreports_missingdogs.getEditText().setText(user_req_adoptdog_approved_adapter.getVet_lokacija());
        boja_myreports_missingdogs.getEditText().setText(user_req_adoptdog_approved_adapter.getBoja());

        napomena_userreq_missingdog.getEditText().setText(user_req_adoptdog_approved_adapter.getNapomena());

        Date date = new Date(user_req_adoptdog_approved_adapter.getPostavljeno());
        DateFormat format = new SimpleDateFormat("dd.MM.yyyy.");
        dostavljeno_nestanak_myreports_missingdogs_.getEditText().setText(format.format(date));

        if(user_req_adoptdog_approved_adapter.getUrl_slike()!=null) {
            slika_myreports_missingdogs.setVisibility(View.VISIBLE);
            Picasso.get().load(user_req_adoptdog_approved_adapter.getUrl_slike()).into(slika_myreports_missingdogs);
        }
        else{
...
//                System.out.println("aaa:"+radioButton.getText().toString());
                try {
                    if (radioButton.getText().toString().equals("Prihvati")) {
                        ...
                        Call<updateuserreq_missingdog[]> call=retrofitInterface.updatereportadopteddog(user_req_adoptdog_approved_adapter.getid(), data);
...
//                                    userreq_adoptdog_req.setListItems(user_req_adoptdog_approved_adapter.getChange());
                                    Toast.makeText(getActivity(),"Zahtjev ažuriran!",Toast.LENGTH_SHORT).show();
                                    userreq_adoptdog_data data = new userreq_adoptdog_data(
                                            user_req_adoptdog_approved_adapter.getIme(),
                                            user_req_adoptdog_approved_adapter.getPrezime(),
                                            user_req_adoptdog_approved_adapter.getAdresa(),
                                            user_req_adoptdog_approved_adapter.getTelefonskibr(),
                                            user_req_adoptdog_approved_adapter.getGrad(),
                                            user_req_adoptdog_approved_adapter.getPostanski_broj(),
                                            user_req_adoptdog_approved_adapter.getBoja(),
                                            user_req_adoptdog_approved_adapter.getStarost(),
                                            user_req_adoptdog_approved_adapter.getDlaka(),
                                            user_req_adoptdog_approved_adapter.getVet_lokacija(),
                                            user_req_adoptdog_approved_adapter.getIme_psa(),
                                            user_req_adoptdog_approved_adapter.getSpol(),
                                            user_req_adoptdog_approved_adapter.getPasmina(),
                                            user_req_adoptdog_approved_adapter.getKilaza(),
                                            user_req_adoptdog_approved_adapter.getKastrat(),
                                            user_req_adoptdog_approved_adapter.getOpasnost(),
                                            user_req_adoptdog_approved_adapter.getNapomena(),
                                            user_req_adoptdog_approved_adapter.getUrl_slike(),
                                            azilnapomena_userreq_adopteddog.getEditText().getText().toString(),
                                            "true", user_req_adoptdog_approved_adapter.getPostavljeno(),
                                            user_req_adoptdog_approved_adapter.getid(),
                                            user_req_adoptdog_approved_adapter.getUdomljavanje_id());
                                    userreq_adoptdog_approve.setListItemschange(user_req_adoptdog_approved_adapter.getChange(), data);
                                }
                            }
                        ...
                        call.enqueue(new Callback<updateuserreq_missingdog[]>() {
                            ...
                            public void onResponse(Call<updateuserreq_missingdog[]> call, Response<updateuserreq_missingdog[]> response) {
                                if(response.code()==200){
                                    userreq_adoptdog_approve.setListItems(user_req_adoptdog_approved_adapter.getChange());
                                    Toast.makeText(getActivity(),"Zahtjev ažuriran!",Toast.LENGTH_SHORT).show();
                                }
                            }
