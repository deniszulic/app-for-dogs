...
public class missingdog_userreject_dialog extends BottomSheetDialogFragment {
...
//    private Button send_myreports_missingdogs;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if(user_req_missingdog_rejectadapter.getPrihvaceno().equals("false")){
            radio_button_2_userreq_missingdog.setChecked(true);
        }

        ime_nestanak_myreports_missingdogs.getEditText().setText(user_req_missingdog_rejectadapter.getIme());
        prezime_nestanak_myreports_missingdogs.getEditText().setText(user_req_missingdog_rejectadapter.getPrezime());
        telefonskibr_myreports_missingdogs.getEditText().setText(user_req_missingdog_rejectadapter.getTelefonskibr());
        grad_azila_myreports_missingdogs.getEditText().setText(user_req_missingdog_rejectadapter.getGrad());
        postanski_broj_myreports_missingdogs.getEditText().setText(user_req_missingdog_rejectadapter.getPostanski_broj());
        napomena_nestanak_myreports_missingdogs.getEditText().setText(user_req_missingdog_rejectadapter.getNestanak_napomena());
        dlaka_myreports_missingdogs.getEditText().setText(user_req_missingdog_rejectadapter.getDlaka());
        spol_myreports_missingdogs.getEditText().setText(user_req_missingdog_rejectadapter.getSpol());
        ...
        vetlokacija_myreports_missingdogs.getEditText().setText(user_req_missingdog_rejectadapter.getVet_lokacija());
        boja_myreports_missingdogs.getEditText().setText(user_req_missingdog_rejectadapter.getBoja());
        napomena_userreq_missingdog.getEditText().setText(user_req_missingdog_rejectadapter.getAzil_nestanak_napomena());

        Date date = new Date(user_req_missingdog_rejectadapter.getPostavljeno());
        DateFormat format = new SimpleDateFormat("dd.MM.yyyy.");
        dostavljeno_nestanak_myreports_missingdogs_.getEditText().setText(format.format(date));

        if(user_req_missingdog_rejectadapter.getUrl_slike()!=null) {
            slika_myreports_missingdogs.setVisibility(View.VISIBLE);
            Picasso.get().load(user_req_missingdog_rejectadapter.getUrl_slike()).into(slika_myreports_missingdogs);
        }
        else{
        ...
        datum=user_req_missingdog_rejectadapter.getDatum_izgubljen();
        materialDateBuilder.setSelection(user_req_missingdog_rejectadapter.getDatum_izgubljen().getTime()+2*HOUR);
        ...
        spremi_userreq_missingdog.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(radioButton.getText().toString().equals("Prihvati")) {
                    ...
                    Call<updateuserreq_missingdog[]> call=retrofitInterface.updatereportmissingdog(user_req_missingdog_rejectadapter.getid(), data);
                            ...
                            if(response.code()==200){
                                try{
                                    ...
                                    userreq_missingdog_data data = new userreq_missingdog_data(user_req_missingdog_rejectadapter.getIme(),
                                            ...
                                            user_req_missingdog_rejectadapter.getAdresa(),
                                            user_req_missingdog_rejectadapter.getTelefonskibr(),
                                            user_req_missingdog_rejectadapter.getGrad(),
                                            user_req_missingdog_rejectadapter.getPostanski_broj(),
                                            user_req_missingdog_rejectadapter.getBoja(),
                                            user_req_missingdog_rejectadapter.getStarost(),
                                            user_req_missingdog_rejectadapter.getDlaka(),
                                            user_req_missingdog_rejectadapter.getVet_lokacija(),
                                            user_req_missingdog_rejectadapter.getIme_psa(),
                                            user_req_missingdog_rejectadapter.getSpol(),
                                            user_req_missingdog_rejectadapter.getPasmina(),
                                            user_req_missingdog_rejectadapter.getDatum_izgubljen(),
                                            user_req_missingdog_rejectadapter.getNestanak_napomena(),
                                            user_req_missingdog_rejectadapter.getUrl_slike(),
                                            napomena_userreq_missingdog.getEditText().getText().toString(),
                                            "false", user_req_missingdog_rejectadapter.getid(),
                                            user_req_missingdog_rejectadapter.getAzil_nestanak_nestanak_id(),
                                            user_req_missingdog_rejectadapter.getPostavljeno(),
                                            user_req_missingdog_rejectadapter.getNestanak_id());
                                    userreq_mis_reject.setListItemschange(user_req_missingdog_rejectadapter.getChange(), data);
                                }catch(Exception e){System.out.println(e);}
                            }
                        }
