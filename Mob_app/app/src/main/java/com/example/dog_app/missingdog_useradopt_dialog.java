...
public class missingdog_useradopt_dialog extends BottomSheetDialogFragment {
...
//        send_myreports_missingdogs = view.findViewById(R.id.send_myreports_missingdogs);
        ...
        if(user_req_missingdog_adopadapter.getPrihvaceno().equals("true")){
            radio_button_1_userreq_missingdog.setChecked(true);
        }
        if(user_req_missingdog_adopadapter.getPrihvaceno().equals("false")){
            radio_button_2_userreq_missingdog.setChecked(true);
        }

        ime_nestanak_myreports_missingdogs.getEditText().setText(user_req_missingdog_adopadapter.getIme());
        prezime_nestanak_myreports_missingdogs.getEditText().setText(user_req_missingdog_adopadapter.getPrezime());
        telefonskibr_myreports_missingdogs.getEditText().setText(user_req_missingdog_adopadapter.getTelefonskibr());
        grad_azila_myreports_missingdogs.getEditText().setText(user_req_missingdog_adopadapter.getGrad());
        postanski_broj_myreports_missingdogs.getEditText().setText(user_req_missingdog_adopadapter.getPostanski_broj());
        napomena_nestanak_myreports_missingdogs.getEditText().setText(user_req_missingdog_adopadapter.getNestanak_napomena());
        dlaka_myreports_missingdogs.getEditText().setText(user_req_missingdog_adopadapter.getDlaka());
        spol_myreports_missingdogs.getEditText().setText(user_req_missingdog_adopadapter.getSpol());


        adresa_myreports_missingdogs.getEditText().setText(user_req_missingdog_adopadapter.getAdresa());
//        grad_myreports_missingdogs.getEditText().setText(user_req_missingdog_adopadapter.getGrad());
//        postnum_myreports_missingdogs.getEditText().setText(user_req_missingdog_adopadapter.getPostnum());
        imepsa_myreports_missingdogs.getEditText().setText(user_req_missingdog_adopadapter.getIme_psa());
        pasmina_myreports_missingdogs.getEditText().setText(user_req_missingdog_adopadapter.getPasmina());
        starost_myreports_missingdogs.getEditText().setText(user_req_missingdog_adopadapter.getStarost());
        vetlokacija_myreports_missingdogs.getEditText().setText(user_req_missingdog_adopadapter.getVet_lokacija());
        boja_myreports_missingdogs.getEditText().setText(user_req_missingdog_adopadapter.getBoja());
        napomena_userreq_missingdog.getEditText().setText(user_req_missingdog_adopadapter.getAzil_nestanak_napomena());

        Date date = new Date(user_req_missingdog_adopadapter.getPostavljeno());
        DateFormat format = new SimpleDateFormat("dd.MM.yyyy.");
        dostavljeno_nestanak_myreports_missingdogs_.getEditText().setText(format.format(date));

        if(user_req_missingdog_adopadapter.getUrl_slike()!=null) {
            slika_myreports_missingdogs.setVisibility(View.VISIBLE);
            Picasso.get().load(user_req_missingdog_adopadapter.getUrl_slike()).into(slika_myreports_missingdogs);
        }
        else{
        ...
        datum=user_req_missingdog_adopadapter.getDatum_izgubljen();
        materialDateBuilder.setSelection(user_req_missingdog_adopadapter.getDatum_izgubljen().getTime()+2*HOUR);
        ...
        spremi_userreq_missingdog.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(radioButton.getText().toString().equals("Prihvati")) {
                    ...
                    Call<updateuserreq_missingdog[]> call=retrofitInterface.updatereportmissingdog(user_req_missingdog_adopadapter.getid(), data);
