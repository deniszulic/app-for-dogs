...
public class adopteddog_userreq_dialog extends BottomSheetDialogFragment {
...
//        napomena_nestanak_myreports_missingdogs=(TextInputLayout) view.findViewById(R.id.azilnapomena_userreq_adopteddog);
        ...
        if(user_req_adoptdog_waiting_adapter.getPrihvaceno().equals("true")){
            radio_button_1_userreq_missingdog.setChecked(true);
        }
        if(user_req_adoptdog_waiting_adapter.getPrihvaceno().equals("false")){
...
//        }
        ...
        ime_nestanak_myreports_missingdogs.getEditText().setText(user_req_adoptdog_waiting_adapter.getIme());
        prezime_nestanak_myreports_missingdogs.getEditText().setText(user_req_adoptdog_waiting_adapter.getPrezime());
        telefonskibr_myreports_missingdogs.getEditText().setText(user_req_adoptdog_waiting_adapter.getTelefonskibr());
        grad_azila_myreports_missingdogs.getEditText().setText(user_req_adoptdog_waiting_adapter.getGrad());
        postanski_broj_myreports_missingdogs.getEditText().setText(user_req_adoptdog_waiting_adapter.getPostanski_broj());
        azilnapomena_userreq_adopteddog.getEditText().setText(user_req_adoptdog_waiting_adapter.getAzil_udomljavanje_napomena());
        dlaka_myreports_missingdogs.getEditText().setText(user_req_adoptdog_waiting_adapter.getDlaka());
        spol_myreports_missingdogs.getEditText().setText(user_req_adoptdog_waiting_adapter.getSpol());


        adresa_myreports_missingdogs.getEditText().setText(user_req_adoptdog_waiting_adapter.getAdresa());
//        grad_myreports_missingdogs.getEditText().setText(user_req_adoptdog_waiting_adapter.getGrad());
//        postnum_myreports_missingdogs.getEditText().setText(user_req_adoptdog_waiting_adapter.getPostnum());
        imepsa_myreports_missingdogs.getEditText().setText(user_req_adoptdog_waiting_adapter.getIme_psa());
        pasmina_myreports_missingdogs.getEditText().setText(user_req_adoptdog_waiting_adapter.getPasmina());
        starost_myreports_missingdogs.getEditText().setText(user_req_adoptdog_waiting_adapter.getStarost());
        vetlokacija_myreports_missingdogs.getEditText().setText(user_req_adoptdog_waiting_adapter.getVet_lokacija());
        boja_myreports_missingdogs.getEditText().setText(user_req_adoptdog_waiting_adapter.getBoja());

        napomena_userreq_missingdog.getEditText().setText(user_req_adoptdog_waiting_adapter.getNapomena());

        Date date = new Date(user_req_adoptdog_waiting_adapter.getPostavljeno());
        DateFormat format = new SimpleDateFormat("dd.MM.yyyy.");
        dostavljeno_nestanak_myreports_missingdogs_.getEditText().setText(format.format(date));

        if(user_req_adoptdog_waiting_adapter.getUrl_slike()!=null) {
            slika_myreports_missingdogs.setVisibility(View.VISIBLE);
            Picasso.get().load(user_req_adoptdog_waiting_adapter.getUrl_slike()).into(slika_myreports_missingdogs);
        }
        else{
...
//                    System.out.println("tekst0:" + napomena_userreq_missingdog.getEditText().getText().toString());
                        updateuserreq_missingdog data= new updateuserreq_missingdog("obrada", azilnapomena_userreq_adopteddog.getEditText().getText().toString());
                        Call<updateuserreq_missingdog[]> call=retrofitInterface.updatereportadopteddog(user_req_adoptdog_waiting_adapter.getid(), data);
